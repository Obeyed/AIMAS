...
if __name__ == '__main__':
    ...
    print("AgentAt(1,3) should be empty: {0}".format([act[arg] for act in a]))
    a = find_applicable_actions([MOVE], create_literal_dict(AGENT_AT, [(1,1)]))
    print("AgentAt(1,1) should have two actions (N, W): {0}".format([act[arg] for act in a]))
    a = find_applicable_actions([MOVE], create_literal_dict(FREE, [(1,1)]))
    print("Free(1,1) should have two actions (S, E): {0}".format([act[arg] for act in a]))

    print("## PUSH:")
    a = find_applicable_actions([PUSH], create_literal_dict(AGENT_AT, [(1,2)]))
    print("AgentAt(1,2) should be empty: {0}".format([act[arg] for act in a]))
    a = find_applicable_actions([PUSH], create_literal_dict(AGENT_AT, [(1,1)]))
    print("AgentAt(1,1) should have two actions [(W, S), (N, E)]: {0}".format([act[arg] for act in a]))
    a = find_applicable_actions([PUSH], create_literal_dict(BOX_AT, [(2,1)]))
    print("BoxAt(2,1) should have one action (W, S): {0}".format([act[arg] for act in a]))
    a = find_applicable_actions([PUSH], create_literal_dict(FREE, [(1,2)]))
    print("Free(1,2) should have one action (W, S): {0}".format([act[arg] for act in a]))
    a = find_applicable_actions([PUSH], create_literal_dict(FREE, [(1,1)]))
    print("Free(1,1) should be empty: {0}".format([act[arg] for act in a]))

    print("## PULL:")
    a = find_applicable_actions([PULL], create_literal_dict(AGENT_AT, [(1,2)]))
    print("AgentAt(1,2) should have one action (E, S): {0}".format([act[arg] for act in a]))
    a = find_applicable_actions([PULL], create_literal_dict(AGENT_AT, [(1,1)]))
    print("AgentAt(1,1) should be empty: {0}".format([act[arg] for act in a]))
    a = find_applicable_actions([PULL], create_literal_dict(BOX_AT, [(1,1)]))
    print("BoxAt(1,1) should have two actions [(E, S), (S, E)]: {0}".format([act[arg] for act in a]))
    a = find_applicable_actions([PULL], create_literal_dict(BOX_AT, [(2,1)]))
    print("BoxAt(2,1) should be empty: {0}".format([act[arg] for act in a]))
    a = find_applicable_actions([PULL], create_literal_dict(FREE, [(2,1)]))
    print("Free(2,1) should have one action (E, S): {0}".format([act[arg] for act in a]))
